...
#Region "Function"
        ...
        Public Function Load(ByVal sNo As String) As Boolean
            ...
            Try
                StrSQL = " SELECT * FROM [Good Menu] WHERE No_ = '" & sNo & "' "
                ...
                If SqlReader.Read() Then
                    ...
                    mPageNo = GetValue(SqlReader, 19, typeOfColumn.GetString)
                    SqlReader.Close()
                    Return True
                Else
                    SqlReader.Close()
        ...
        Public Function LoadDivisionFromItemLink(ByVal sItemLink As String) As Boolean
            Dim StrSQL As String
            Dim SqlReader As IDataReader
            Try
                StrSQL = " SELECT * FROM [Good Menu] WHERE No_ IN (SELECT [Menu Division No_] FROM Item WHERE [Link URL] = '" & sItemLink & "')"
                SqlReader = DBHelper.ExecuteReader(GetConnectString, CommandType.Text, StrSQL)
                If SqlReader.Read() Then
                    mNo_ = GetValue(SqlReader, 0, typeOfColumn.GetString)
                    mName = GetValue(SqlReader, 1, typeOfColumn.GetString)
        ...
        Public Function LoadFromCat(ByVal sNo As String) As Boolean
            Dim StrSQL As String
            Dim SqlReader As IDataReader
            Try
                StrSQL = " SELECT * FROM [Good Menu] WHERE No_ = (SELECT [Parent No_] FROM [Good Menu] WHERE No_ ='" & sNo & "')"
                SqlReader = DBHelper.ExecuteReader(GetConnectString, CommandType.Text, StrSQL)
                If SqlReader.Read() Then
                    mNo_ = GetValue(SqlReader, 0, typeOfColumn.GetString)
                    mName = GetValue(SqlReader, 1, typeOfColumn.GetString)
        ...
        Public Function Save() As Boolean
            ...
            Try
                ...
                arParams(19) = DBHelper.createParameter("@PageNo_", SqlDbType.NVarChar, ParameterDirection.Input, mPageNo)
        ...
        Function GetList(Optional ByVal MenuType As String = "", Optional ByVal sPageNo As String = "", Optional ByVal sConditions As String = "") As DataTable
            Dim SQL As String
            SQL = " SELECT * FROM [Good Menu] WHERE 1 = 1 "
            If MenuType.Trim <> "" Then
                SQL &= " AND [Menu Type] = " & MenuType
            End If
            If sPageNo.Trim <> "" Then
                SQL &= " AND [Page No_] = '" & sPageNo & "'"
            End If
