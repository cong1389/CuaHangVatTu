/* Adjust Width According Browser's Width */

 function adjustStyle(width) {

	var css_1024 = J_BASE_URL + "frontend/default/narrow/narrow.css";

    width = parseInt(width);
    if (width <= 1200) {
        jQuery("#width_adjust").attr("href", css_1024);
    } else {
       jQuery("#width_adjust").removeAttr("href", ""); 
    }
}
...
jQuery(function () {
	jQuery(".block-login, .currency-block").hover(function () {
...
/* Jcarousel global window, jQuery  for home page special products */
(function($) {
...
/* quantity increments ¨C jQuery edition    http://jigowatt.co.uk/blog/magento-quantity-increments-jquery-edition  */

function ChangerAnimate() {
    var quantityInput = jQuery("#qty");
    var quantity = parseInt(quantityInput.val());
    if (!isNaN(quantity)) {
        if (quantity == 1) {
            jQuery("#minus1").addClass("un");
        } else {
            jQuery("#minus1").removeClass("un");
        }
    }
}
...
jQuery("div.quantity").append('<input type="button" value="+" id="add1" class="plus" />').prepend('<input type="button" value="-" id="minus1" class="minus un" />');
...
function wppShowMenuPopup(objMenu, popupId) {
    objMenu = document.getElementById(objMenu.id);
    var popup = document.getElementById(popupId);
    popup.style.display = 'block';
    objMenu.addClassName('actiwe');
    var popupWidth = CUSTOMMENU_POPUP_WIDTH;
    if (!popupWidth) popupWidth = popup.getWidth();
    var pos = wppPopupPos(objMenu, popupWidth);
    if (TOP_YES == '1')
    { pos.top = '0'; }
